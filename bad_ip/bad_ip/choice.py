...
class choice:
    def Sing_RequestQQGJ(IP):
        ...
        print(IP)
        response = (requests.get(url=url, proxies=proxy, headers=headers,timeout=30).text.encode('utf-8').decode("unicode_escape"))
        print(response)
        ip = re.findall(r'\\\/\\\/(.*),"whitetype"', response)

        wordingtitle = re.findall(r'"WordingTitle":(.*),"Wording"', response)
        #wording = re.findall(r'"Wording":(.*),"detect_time"', response)
        result = ip + wordingtitle
        return str(result)
        #return str(choice.HandleSingleResult(response))
